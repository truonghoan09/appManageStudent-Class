    ...
    const saveFunc = async() => {
        ...
        cloneNextLessons.map((v,i) => {
            preSchedule.map((vSnap, iSnap) => {
                if (Math.abs(v.date - new Date(vSnap.date)) < 1000*60*60){
                    // cloneNextLessons.splice(i, 1);
        ...
        if (cloneRedNoticePreSchedule.indexOf(true) === -1) {
                dispatch(updateScheduleNextLessonsAction(localStorage.getItem("uid"), localStorage.getItem("classChoosen"), nextLessons))
                dispatch(onModalUpdateNextLessonsAction(false))
                location.reload();
        } else {
            setNoticeMessage('Please choose a different time slot at least 1 hour away from the current lesson.');
                        ...
                        <div className={styles.containerBlockDayChoosen}>
                            <div className={styles.blockChooseDay}>
                                ...
                                <div className={`${styles.pickTimeTag} ${redNotice[i] && styles.redNotice}`}>
                                    <DateTimePicker format="dd/MM/yyyy hh:mm a" clearIcon={null}  minDate={currentDate} disableClock onChange={(value) => changeDate(i, value)} value={v} />
                                </div>
                            </div>
